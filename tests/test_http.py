...
class HttpTest(unittest.TestCase):
    def serve(self, store, config=None, **fakes):
        self.store = store
        server = make_server(port=0, store=store, config=config, **fakes)
    ...
    def test_health(self):
        ...
        self.assertEqual(set(r), {"ok", "checks", "config", "home", "egress"})
    ...
    def test_find_roundtrip(self):
        ...
        self.serve(make_store(self), config=cfg, runner=runner, jev=fake_jev([0.9, 0.8, 0.1]))
        ...
        self.assertEqual(r, {"store": str(self.store), "sources": [str(src.resolve())],
                             "embedding": MULTILINGUAL_MODEL, "files": 1, "excluded": 0})
        self.assertEqual(runner.calls[-1][0][-2:], ["--hidden", "--rebuild"])

    def test_ask_language(self):
        runner = FakeRunner()
        cfg = make_config(self)
        self.serve(make_store(self), config=cfg, runner=runner, jev=fake_jev([0.9, 0.2, 0.1]))
    ...
    def test_rejects_store_and_unknown_keys(self):
        runner = FakeRunner()
        cfg = make_config(self)
        self.serve(make_store(self), config=cfg, runner=runner, jev=fake_jev([]))
    ...
    def test_library_error_is_422(self):
        ...
        self.serve(empty.name, config=make_config(self), runner=FakeRunner(), jev=fake_jev([]))
        code, r = self.call("/find", {"query": "q"})
        self.assertEqual(code, 422)
        self.assertIn("no index", r["error"])

    def test_404_405_413(self):
        self.serve(make_store(self), config=make_config(self), runner=FakeRunner(), jev=fake_jev([]))
